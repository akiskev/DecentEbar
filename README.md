...
- **Pressure LUT** — JSON import/export, screen resolution + orientation + package validation, nearest-point lookup, linear interpolation, command throttling, and min/max pressure clamping
...
| **LUT** | LUT status, pressure test slider, JSON import/export |
...
1. **Preinfusion** — 2 bar fixed, exits at first drop (0.3 g) or 20 s
...
3. In the **LUT** tab, import a pressure LUT JSON that matches your E-Bar screen resolution and orientation.
4. Select or create a shot profile in the **Profile** tab and tap **Save**.
5. In the **Control** tab, tap **Arm**, then start a shot in the E-Bar app — the controller takes over automatically.
